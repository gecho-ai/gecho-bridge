---
name: amazon-reviews
description: 通过官方 Gecho Bridge MCP 采集 Amazon 商品评论，包括评论文本、评分、日期和可用的评论者信息。用户提供 Amazon 商品 URL 或 ASIN 并需要评论调研时使用。
metadata:
  openclaw:
...
使用官方 Gecho Bridge MCP 工具采集一个 Amazon 商品的评论。本 Skill 处理评论采集；商品发现使用 `amazon-search`，商品规格使用 `amazon-product`。
...
完成设置后，返回 OpenClaw Dashboard 或 Hermes，提问：“采集这个 Amazon 商品的评论：https://www.amazon.com/dp/B0CXJJHY8B”。
...
## 本 Skill 能做什么
- 将请求路由到官方 amazon_reviews 工作流。
- 采集结构化 Amazon 数据，并尽可能保存原始结果。
- 在不淹没对话的前提下总结有用字段和链接。
- 明确处理浏览器、登录、验证码、超时和保存失败。

适用提示词：
- “采集这个 Amazon 商品的评论：https://www.amazon.com/dp/B0CXJJHY8B”
- “采集结构化结果并保存到我的调研目录。”
...
核心规则：
- 本流程仅使用官方 Gecho MCP `amazon_reviews` 工具。
