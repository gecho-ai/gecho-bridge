---
name: x-post-detail
description: 通过官方 Gecho Bridge MCP 获取一条 X（Twitter）帖子及其回复。用户提供 X 帖子 URL，并需要正文、作者、互动数据或评论时使用。
metadata:
  openclaw:
...
使用官方 Gecho Bridge MCP 工具采集一条 X 帖子及其可获取的回复。本 Skill 处理已知帖子 URL；关键词发现使用 `x-search`。
...
完成设置后，返回 OpenClaw Dashboard 或 Hermes，提问：“采集这条 X 帖子及其回复：https://x.com/example/status/123”。
...
- `x-search`：按关键词发现 X 帖子。
- `x`：X 搜索与帖子详情的完整调研。
- `tiktok-video-search`：TikTok 视频搜索与元数据采集。
- `amazon-reviews`：Amazon 商品评论调研。
...
核心规则：
- 本流程仅使用官方 Gecho MCP `x_post_detail` 工具。
