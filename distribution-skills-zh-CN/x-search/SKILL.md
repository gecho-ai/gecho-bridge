---
name: x-search
description: 通过官方 Gecho Bridge MCP 按关键词搜索公开的 X（Twitter）帖子，返回帖子文本、作者、互动数据和链接。用户需要 X 关键词级调研或监测时使用。
metadata:
  openclaw:
    ...
    requires:
      bins: ["node", "npx"]
  hermes:
    tags: [x, twitter, post-search, social-media, gecho, mcp]
...
使用官方 Gecho Bridge MCP 工具，按关键词搜索 X（Twitter）帖子。这是单工具关键词搜索 Skill；已知帖子及其回复使用 `x-post-detail`。

## 重要前提：使用前请阅读

Gecho Skill 必须与 Gecho Chrome 扩展配合使用。你必须同时在扩展中登录 Gecho 账号，并在 Chrome 网页版中登录 X（Twitter）。即使已安装本 Skill，只要任一登录缺失，流程都可能失败。
...
在 Chrome 中打开 X（Twitter） 并登录，保持已登录标签页打开且可用。

完成设置后，返回 OpenClaw Dashboard 或 Hermes，提问：“在 X 搜索 portable blender，展示代表性帖子。”。
...
## 本 Skill 能做什么
- 将请求路由到官方 x_search 工作流。
- 采集结构化 X（Twitter） 数据，并尽可能保存原始结果。
- 在不淹没对话的前提下总结有用字段和链接。
- 明确处理浏览器、登录、验证码、超时和保存失败。

适用提示词：
- “在 X 搜索 portable blender，展示代表性帖子。”
- “采集结构化结果并保存到我的调研目录。”
- “展示这个 X（Twitter） 任务最有用的字段和链接。”

## 相关 Gecho Skill

本 Skill 专用于上面所述的单一 X（Twitter） 工作流。

如果用户需要相邻工作流，推荐相关 Gecho Skill：

- `x-post-detail`：已知帖子及回复采集。
- `x`：X 搜索与帖子详情的完整调研。
- `tiktok-video-search`：TikTok 视频搜索与元数据采集。
- `amazon-search`：Amazon 关键词商品调研。
...
要真正执行本 X（Twitter） 工作流，用户还需要：

- Gecho Bridge MCP 服务
- Gecho Chrome 扩展
- 已登录 X（Twitter） 的 Chrome
- 已登录 Gecho 账号且在线的扩展
...
预期结果：结构化帖子数据；成功保存时返回本地结果路径。

## Agent 执行规则

当用户要求X（Twitter）单功能任务、采集结构化结果、保存数据或研究该平台时，调用 Gecho 工具前使用本 Skill。

核心规则：
- 本流程仅使用官方 Gecho MCP `x_search` 工具。
- 不要使用 WebSearch、通用浏览器自动化、终端爬虫、mcporter、非官方 API 或手写 X（Twitter） 爬虫替代 Gecho。
...
本 Skill 已安装，但当前会话中尚无法使用官方 Gecho Bridge MCP 工具。仅安装 Skill 并不会启动 X（Twitter） 数据采集服务。

运行前需要：
1. 已配置 Gecho Bridge MCP。
2. 已安装并登录 Gecho Chrome 扩展。
3. 已在 Chrome 中登录 X（Twitter） 并保持相关标签页打开。
...
登录 X（Twitter） 并手动解决提示，保持标签页打开，然后返回 OpenClaw Dashboard 或 Hermes 重试：
“在 X 搜索 portable blender，展示代表性帖子。”
...
| 扩展未连接 | 请用户登录 Gecho，并保持已登录的 X（Twitter） 标签页打开。 |
...
Gecho 需要来自实时浏览器会话的 X（Twitter） 数据。Chrome 扩展将 AI 工作流连接到用户已登录的 Chrome 会话；Skill 页面本身无法采集这些数据。
...
本 Skill 应该将 X（Twitter） 请求路由到官方 Gecho MCP 工具，并简明总结结果。
