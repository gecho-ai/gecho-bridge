---
name: tiktok-product
description: 通过官方 Gecho Bridge MCP 获取完整的 TikTok Shop 商品详情，包括标题、价格、SKU、描述、销量、评分和评价。用户提供 TikTok Shop 商品 URL 或商品 ID 时使用。
metadata:
  openclaw:
...
# Gecho TikTok Shop 商品详情

使用官方 Gecho Bridge MCP 工具采集一个 TikTok Shop 商品的详情。本 Skill 只处理商品详情页；关键词发现使用 `tiktok-shop-search`。
...
完成设置后，返回 OpenClaw Dashboard 或 Hermes，提问：“获取这个 TikTok Shop 商品的详情：https://shop.tiktok.com/us/pdp/example”。
...
## 本 Skill 能做什么
- 将请求路由到官方 tiktok_product 工作流。
- 采集结构化 TikTok Shop 数据，并尽可能保存原始结果。
- 在不淹没对话的前提下总结有用字段和链接。
- 明确处理浏览器、登录、验证码、超时和保存失败。

适用提示词：
- “获取这个 TikTok Shop 商品的详情：https://shop.tiktok.com/us/pdp/example”
- “采集结构化结果并保存到我的调研目录。”
...
核心规则：
- 本流程仅使用官方 Gecho MCP `tiktok_product` 工具。
