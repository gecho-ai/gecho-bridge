---
name: amazon-search
description: 通过官方 Gecho Bridge MCP 按关键词搜索 Amazon 商品，返回指定站点的商品列表、价格、评分和链接。用户需要 Amazon 商品发现或目录调研时使用。
metadata:
  openclaw:
...
使用官方 Gecho Bridge MCP 工具按关键词搜索 Amazon 商品。这是单工具发现 Skill；已知商品使用 `amazon-product`，评论采集使用 `amazon-reviews`。
...
完成设置后，返回 OpenClaw Dashboard 或 Hermes，提问：“在 Amazon US 搜索 portable blender，展示热门商品。”。
...
## 本 Skill 能做什么
- 将请求路由到官方 amazon_search 工作流。
- 采集结构化 Amazon 数据，并尽可能保存原始结果。
- 在不淹没对话的前提下总结有用字段和链接。
- 明确处理浏览器、登录、验证码、超时和保存失败。

适用提示词：
- “在 Amazon US 搜索 portable blender，展示热门商品。”
- “采集结构化结果并保存到我的调研目录。”
...
- `amazon-reviews`：已知商品评论采集。
- `amazon`：Amazon 搜索、商品详情和评论的完整调研。
- `tiktok-shop-search`：TikTok Shop 商品发现。
...
核心规则：
- 本流程仅使用官方 Gecho MCP `amazon_search` 工具。
