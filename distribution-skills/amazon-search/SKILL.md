---
name: amazon-search
description: Search Amazon products by keyword with the official Gecho Bridge MCP tool and return structured listings, prices, ratings, and links across a selected marketplace. Use for Amazon product discovery and catalog research.
metadata:
  openclaw:
    ...
    requires:
      bins: ["node", "npx"]
  hermes:
    tags: [amazon, product-search, ecommerce, marketplace, gecho, mcp]
    category: ecommerce
...
Use the official Gecho Bridge MCP tool to search Amazon listings by keyword. This is a single-tool Skill for discovery; use `amazon-product` or `amazon-reviews` for a known item.
...
After setup is complete, return to the OpenClaw dashboard or Hermes and ask: "Search Amazon US for portable blender and show the top listings.".
...
- Routes requests to the official amazon_search workflow.
...
- `amazon-reviews`: review collection for a known product.
- `amazon`: combined Amazon discovery, product-detail, and review research.
- `tiktok-shop-search`: TikTok Shop product discovery.
...
Expected result: structured listing data and, when saved successfully, a local result path.
...
- Use the official Gecho MCP `amazon_search` tool for this workflow.
