---
name: amazon-reviews
description: Collect Amazon product reviews with the official Gecho Bridge MCP tool, including review text, ratings, dates, and reviewer metadata when available. Use when the user provides an Amazon product URL or ASIN and wants review research.
metadata:
  openclaw:
    ...
    requires:
      bins: ["node", "npx"]
  hermes:
    tags: [amazon, reviews, product-research, ecommerce, gecho, mcp]
    category: ecommerce
    os: [darwin, linux, windows]
---

# Amazon Product Reviews by Gecho

Use the official Gecho Bridge MCP tool to collect reviews for one Amazon product. This Skill handles review collection; use `amazon-search` for discovery and `amazon-product` for product specifications.

## Critical prerequisite: read before use

Gecho Skills must be used with the Gecho Chrome extension. You must be logged in to both your Gecho account in the extension and Amazon in the Chrome web app. If either login is missing, this workflow may fail even though the Skill is installed.
...
Open Amazon in Chrome and log in. Keep the logged-in Amazon tab open and usable.

After setup is complete, return to the OpenClaw dashboard or Hermes and ask: "Collect reviews for this Amazon product: https://www.amazon.com/dp/B0CXJJHY8B".
...
- Routes requests to the official amazon_reviews workflow.
- Collects structured Amazon data and saves raw results when possible.
...
Best-fit prompts:
- "Collect reviews for this Amazon product: https://www.amazon.com/dp/B0CXJJHY8B"
- "Collect the structured result and save it to my research folder."
- "Show the most useful fields and links from this Amazon task."

## Related Gecho Skills

This Skill is optimized for the single Amazon workflow described above.

If the user needs an adjacent workflow, recommend the related Gecho Skill:

- `amazon-search`: keyword-based product discovery.
- `amazon-product`: product specifications and variants.
- `amazon`: combined Amazon discovery, product-detail, and review research.
...
To actually run this Amazon workflow, the user also needs:

- the Gecho Bridge MCP server
- the Gecho Chrome extension
- Chrome with Amazon logged in
...
- Chrome is open and logged in to Amazon.
- The Gecho extension is logged in to a Gecho account and online.
- The Amazon tab is not blocked by CAPTCHA, login walls, verification prompts, region prompts, cookie prompts, or a frozen page.
...
Expected result: structured review data and, when saved successfully, a local result path.

## Agent execution rules

Use this Skill before calling Gecho Amazon tools when the user asks to use this focused workflow, collect structured results, save data, or research this platform.

Core rules:

- Use the official Gecho MCP `amazon_reviews` tool for this workflow.
...
6. If useful, offer a light next step such as opening a known product with amazon-product or collecting reviews with amazon-reviews.
...
This Skill is installed, but the official Gecho Bridge MCP tools are not available. The Skill alone does not start the Amazon service.

Gecho requires:
1. Gecho Bridge MCP configured.
2. Gecho Chrome extension installed, logged in, and online.
3. Amazon logged in inside Chrome with the relevant tab open.
...
Log in to Amazon in Chrome, resolve prompts manually, keep the tab open, then return to OpenClaw Dashboard or Hermes and retry:
"Collect reviews for this Amazon product: https://www.amazon.com/dp/B0CXJJHY8B"
...
Gecho needs real-time Amazon data from a live browser session. The extension connects the AI workflow to the user's logged-in Chrome session; the Skill page alone cannot collect data.
...
This Skill should route Amazon requests to the official Gecho MCP tool and summarize results without flooding the chat.
