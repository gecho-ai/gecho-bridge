---
name: amazon-product
description: Retrieve complete Amazon product details with the official Gecho Bridge MCP tool, including title, price, variants, specifications, description, ratings, and links. Use when the user provides an Amazon product URL or ASIN.
metadata:
  openclaw:
...
Use the official Gecho Bridge MCP tool to collect one Amazon product detail page. This Skill handles a known product URL or ASIN; use `amazon-search` for discovery.
...
After setup is complete, return to the OpenClaw dashboard or Hermes and ask: "Get the details for this Amazon product: https://www.amazon.com/dp/B0CXJJHY8B".
...
- Routes requests to the official amazon_product workflow.
...
Best-fit prompts:
- "Get the details for this Amazon product: https://www.amazon.com/dp/B0CXJJHY8B"
...
- `tiktok-product`: TikTok Shop product details.
...
Expected result: structured product details such as title, price, variants, specifications, description, ratings, and links when available, plus a local result path when saved.
...
- Use the official Gecho MCP `amazon_product` tool for this workflow.
