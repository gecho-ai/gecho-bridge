---
name: tiktok-product
description: Retrieve complete TikTok Shop product details with the official Gecho Bridge MCP tool, including title, price, SKU, description, sales, ratings, and reviews. Use when the user provides a TikTok Shop product URL or product ID.
metadata:
  openclaw:
    ...
    requires:
      bins: ["node", "npx"]
  hermes:
    tags: [tiktok, shop, product, ecommerce, gecho, mcp]
    category: ecommerce
    os: [darwin, linux, windows]
---

# TikTok Shop Product Details by Gecho

Use the official Gecho Bridge MCP tool to collect the details of one TikTok Shop product. This Skill handles product detail pages only; use `tiktok-shop-search` for keyword discovery.
...
After setup is complete, return to the OpenClaw dashboard or Hermes and ask: "Get the details for this TikTok Shop product: https://shop.tiktok.com/us/pdp/example".
...
- Routes requests to the official tiktok_product workflow.
...
Best-fit prompts:
- "Get the details for this TikTok Shop product: https://shop.tiktok.com/us/pdp/example"
...
- `tiktok-shop-search`: keyword-based product discovery.
...
Expected result: structured product details such as title, price, SKU/variants, description, sales, ratings, reviews, and links when available, plus a local result path when saved.
...
- Use the official Gecho MCP `tiktok_product` tool for this workflow.
