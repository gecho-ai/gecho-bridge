---
name: x-search
description: Search public X (Twitter) posts by keyword with the official Gecho Bridge MCP tool and return post text, authors, engagement data, and links. Use for keyword-level X research and monitoring.
metadata:
  openclaw:
...
Use the official Gecho Bridge MCP tool to search X (Twitter) for posts matching a keyword. This is a single-tool Skill for keyword search; use `x-post-detail` for one post and its replies.
...
After setup is complete, return to the OpenClaw dashboard or Hermes and ask: "Search X for portable blender and show representative posts.".
...
- Routes requests to the official x_search workflow.
...
Expected result: structured post data and, when saved successfully, a local result path.
...
- Use the official Gecho MCP `x_search` tool for this workflow.
